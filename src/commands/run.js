...
function loadTemplate(inputDir, templatePath) {
  const template = fs.readFileSync(path.join(inputDir, templatePath), 'utf8');
  return template.replace(/\[\[ ([\W\w]+?) \]\]/g, (match, p) => loadTemplate(inputDir, p));
}
...
function paste(obj, template) {
  ...
  result = result.replace(dynamicReplace, (match, key, lower, upper, repeatedTemplate) => {
    ...
    if (Array.isArray(arr) && lower) {
      ...
      let list = '';
      for (let i = lower; i < upper && i < arr.length; i += 1) {
...
function run(inputDir, outputDir, extension, head) {
  ...
  Object.keys(indices).forEach((key) => {
    ...
    try {
      ...
      indices[key].forEach((obj, idx, arr) => {
        ...
        const html = `${marker}\n${htmg.buildDocument(paste(obj, template), head)}`;
  ...
  transformTemplates.forEach((p) => {
    const relPath = path.relative(inputDir, p);
    const template = loadTemplate(inputDir, relPath);
    const html = `${marker}\n${htmg.buildDocument(paste(indices, template), head)}`;
    fs.outputFileSync(path.join(outputDir, relPath.slice(0, -3) + '.' + extension), html);
  });
}
